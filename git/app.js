...
app.post('/districts/', authenticateToken, async (request, response) => {
  ...
  const postDistrictQuery = `INSERT INTO district (state_id,district_name,cases,cured,active,deaths)
    VALUES(
        ${stateId}, '${districtName}', ${cured},${cases},${active},${deaths}
    );`
...
app.get(
  ...
  async (request, response) => {
    const {stateId} = request.params
    const getStateStatesQuery = `SELECT 
    SUM(cases),
    SUM(cured),
    SUM(active),
    SUM(deaths)
    ...
    WHERE
       state_id = ${stateId};`
    const stats = await database.get(getStateStatesQuery)
    response.send({
